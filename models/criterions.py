...
def Dice(output, target, eps=1e-5):
    ...
    #CrossEntropyLoss
    #targetce = target.reshape(target.shape[0],target.shape[1],-1)
    #outputce = output.reshape(output.shape[0],output.shape[1],-1)   
    #loss=nn.CrossEntropyLoss()
    #lossCE = loss(outputce,targetce.long())
    #lossCE = -torch.log(output.gather(1,target.view(-1,1),dtype=torch.int64)).mean()
...
def softmax_dice2(output, target):
    ...
    #crossentropyLoss
    #Trans_one_hot
    #label = target.unsqueeze(1).float()
    #one_hot = torch.LongTensor(label.size()[0],4,label.size()[2],label.size()[3],label.size()[4]).cuda()
    #print(one_hot.shape)1,4,128,128,128
    #targetce = one_hot.scatter_(1,label.long().data,1).cuda()
    #print(targetce.shape)1,4,128,128,128
    #label0 = targetce[:, 0, ...]
    #label1 = targetce[:, 1, ...]
    #label2 = targetce[:, 2, ...]
    #label3 = targetce[:, 3, ...]
    ...
    #loss=nn.CrossEntropyLoss()
    #lossCE0 = loss(output,label0)
    #lossCE1 = loss(output,label1)
    #lossCE2 = loss(output,label2)
    #lossCE3 = loss(output,label3)
    return loss1+loss0, loss0.data, loss1.data
