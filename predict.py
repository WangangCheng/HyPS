        ...
        if not use_TTA:
            ...
            if multimodel:
                ...
                if os.path.isfile(load_file1):
                    checkpoint = torch.load(load_file1,map_location=lambda storage, loc: storage,weights_only=False)
                ...
                checkpoint = torch.load(load_file,map_location=lambda storage, loc: storage,weights_only=False)
                model.load_state_dict(checkpoint['state_dict'])
